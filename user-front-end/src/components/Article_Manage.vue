    ...
    <table>
      ...
      <tbody>
        <tr v-for="(article,i) in list">
          ...
          <td><input type="button" value="编辑" @click="toModify(article)"></td>
...
export default {
  ...
  methods: {
    ...
    deleteArticle: function (i){
      fetch('http://localhost:8080/user/article/delete',{
        method: 'post',
        mode: 'cors',
        headers: {
    ...
    loadData: function (req_page,req_page_size){
      fetch('http://localhost:8080/user/article?page='+req_page+'&page_size='+req_page_size,{
        method: 'get',
        headers: {
          "Authorization": GlobalVar.user_auth_data.role+" "+GlobalVar.user_auth_data.token
        },
